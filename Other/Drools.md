...
## 与Java一起使用
```java
FactHandle fs = kieSession.fireAllRules();
kieSession.delete(fs);
kieSession.fireAllRules(new RuleNameEqualsAgendaFilter("name的使用6"));
new RuleNameEqualsAgendaFilter("name的使用6");//根据规则名精确执行所有匹配的规则
new RuleNameStartsWithAgendaFilter("name");//根据规则名开头执行所有匹配的规则
new RuleNameEndsWithAgendaFilter("的使用6");//根据规则结尾执行所有匹配的规则
new RuleNameMatchesAgendaFilter("\\w{0,5}[\\u4e00-\\u9fa5]{0,10}\\d");// 根据指定的正则表达式过滤器激活。
...
[11]: https://github.com/ityouknow/drools-examples/tree/master/spring-boot-drools-db 'drools集成spring boot'
